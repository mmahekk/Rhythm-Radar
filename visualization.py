...
class MainMenu:
    ...
    def get_recommended(self):
        ...
        # playlist_code = \
        print(self.recommended_song.config(self.playlist_link.get()))
        # playlist_uri = playlist_code.split("/")[-1].split("?")[0]

        # p = score.Computations()
        # p.sort_similar_songs(sp, playlist_uri)
...
if __name__ == '__main__':
    ...
    python_ta.check_all(config={
        'extra-imports': ['math', 'playlist', 'tkinter', 'score_computations', 'spotipy', \
                          'spotipy.oauth2'],  # the names (strs) of imported modules
