...
class Channel:
    ...
    def get_other_endpoint(self, curr_song: Song) -> Song:
        """Return the endpoint of this channel that is not equal to the given song.
...
class Playlist:
    ...
    def add_channel(self, id1: SongID, id2: SongID) -> None:
        """Add a new channel between the songs with the two given addresses.
    ...
    def get_dataset_songs(self, sp: Any) -> list[SongID]:
        """Load all the song data from the CSV file.

        This method will return a list of SongIDs for each song from the CSV file.

        Every song is created into a Song object, containing the song id, the artist name, the title of the song,
        danceability, valence, energy, and loudness respectively.~
        """
        list_of_songs = []
    ...
    def get_playlist_songs(self, sp: Any, playlist_URI: Any) -> list[SongID]:
        """Return a list of 20 random song ids from the user's input playlist~.
        """
        # a dictionary mapping a tuple of the song artist and song title to
        user_songs = []

        # take a sample of 20 random songs
    ...
    def get_songs_in_range(self, sp: Any, playlist_URI: Any, user_songs: list[str], dataset_songs: list[str]) -> None:
        ...
        for input_song in user_songs:
            ...
            for set_song in dataset_songs:
                ...
                if check_target(self, sp, playlist_URI, user_songs, dataset_songs):
        ...
        def check_targets(self, sp: Any, playlist_URI: Any, user_songs: list[str], dataset_songs: list[str]) -> bool:
            return (target_danceability - 0.5) <= danceability <= (target_danceability + 0.5) and \
...
if __name__ == '__main__':
    ...
    python_ta.check_all(config={
        'extra-imports': ['sys', 'random', 'csv'],  # the names (strs) of imported modules
